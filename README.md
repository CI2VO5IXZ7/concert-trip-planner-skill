...
5. **规划去程**：FlyAI 搜索大交通（高铁/飞机），预留场馆缓冲
6. **规划当地路线**：高德 REST API 查询机场/车站 → 场馆的实测路线和时间
7. **规划返程交通**：FlyAI 火车双段（凌晨段+早班段）+ 飞机并行搜索，必要时反向规划
8. **输出行程单**：分钟级安排 + 风险评估 + 不可行时给出最早可达时间

> ⚠️ **数据说明**：火车/航班依赖 FlyAI，体验模式下数据可能不完整；12306 fallback 仅在可扫码登录的交互式会话中可用。结果稀少时会标注"需人工复核"，不会直接判无方案。
